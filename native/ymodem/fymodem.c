...
static void do_flush(stream_t* stream) {
	stream_flush(stream);
}
...
/**
  ...
  */
static int32_t ym_rx_packet(stream_t* stream, uint8_t *rxdata,
                            int32_t *rxlen,
 ...
 */
int32_t fymodem_receive(stream_t* stream, uint8_t *rxdata,
                        size_t rxlen,
                        char filename[FYMODEM_FILE_NAME_MAX_LENGTH])
{
  ...
  do {
    ...
    do {
      // receive packets
      int32_t res = ym_rx_packet(stream, rx_packet_data,
                                 &rx_packet_len,
                                 packets_rxed,
...
//-------------------------------------------------------
bool_t fymodem_send(stream_t* stream, uint8_t* txdata, size_t txsize, const char* filename, const char** err_msg)
{
  ...
  do {
    ...
    if (ch == ACK) {
      ...
      if (ch == CRC) {
        ...
        if(ym_send_data_packets(stream, txdata, (uint32_t)txsize, PACKET_RX_TIMEOUT_SEC)) {
            file_done = true;
		}else{
